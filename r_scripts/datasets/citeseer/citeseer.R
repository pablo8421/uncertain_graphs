############################
# 
# Citeseer dataset exploration
# https://paperswithcode.com/dataset/citeseer
# Scientific publications classified into one of six classes.
...
setwd(paste(datsets_location,'citeseer/',sep = ''))
dataset_name <- 'Citeseer'
dataset_color <- '#2b9469'


# Loading data
citeseer_cites <- read.csv(file='citeseer.cites', sep = '\t', header=FALSE)
citeseer_content <- read.csv(file='citeseer.content', sep = '\t', header=FALSE)

# Numerical values to be given to each class
labels_num <- citeseer_content %>%
  select(V3705) %>%
  rename(label=V3705) %>%
  unique()
labels_num$numeric <- 1:nrow(labels_num)

citeseer_labels <- citeseer_content %>%
  select(V1,V3705) %>%
  rename(paper=V1,label=V3705) %>%
  left_join(labels_num, by=c('label'='label'), suffix = c(".x", ".y"))

# Filtering missing nodes in the content data.
# Why is this step necessary?
citeseer_cites <- citeseer_cites %>% 
  filter(V1%in%citeseer_labels$paper) %>%
  filter(V2%in%citeseer_labels$paper)


### Graph creation ###

# Creating the graph object
net <- graph_from_data_frame(d=citeseer_cites, vertices=citeseer_labels, directed=FALSE) 
...
net_modularity <- modularity(net, as_data_frame(net, what='vertices')$nu1meric)
