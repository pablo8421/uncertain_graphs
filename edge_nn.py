...
class EdgeNN(object):
  ...
  def __init__(self):
    self.flag_learn_rate = 0.1
    self.flag_optimizer = 'pd'
  ...
  def build_train_op(self):
    ...
    if len(self.gradients) > 2:
      if self.flag_optimizer == 'adam':
        optimizer = tf.train.AdamOptimizer(self.learn_rate)
      elif self.flag_optimizer == 'pd':
  ...
  def apply_gradients(self, sess, grads, epoch):
    ...
    feed_dict[self.learn_rate] = self.flag_learn_rate
