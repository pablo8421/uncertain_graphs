...
def InFile(suffix):
  ...
  return tf.gfile.Open(ModelFileName(suffix))


def OutFile(suffix):
  """Opens file `ModelFileName(suffix)` for writing."""
  return tf.gfile.Open(ModelFileName(suffix), 'w')
...
class Evaluator(object):
  ...
  def __init__(self, positive_pairs_file, negative_pairs_file):
    self.pos_data = numpy.load(tf.gfile.Open(positive_pairs_file, 'rb'))
    self.neg_data = numpy.load(tf.gfile.Open(negative_pairs_file, 'rb'))
...
class TrainNegatives(object):
  ...
  def __init__(self, train_negatives_file):
    train_negatives_arr = numpy.load(tf.gfile.Open(train_negatives_file,'rb'))
...
class TrainPairsReader(object):
  ...
  def __init__(self):
    files = tf.gfile.Glob(DatasetFileName('train.pairs.*.npy'))
    random.shuffle(files)
    self.train_npy_files = files
    self.next_idx = 0

  def next_pairs_array(self):
    arr = numpy.load(tf.gfile.Open(self.train_npy_files[self.next_idx],'rb'))
...
def main(unused_argv):
  ...
  if FLAGS.num_nodes == 0:
    ...
    print()
    FLAGS.num_nodes = len(pickle.load(tf.gfile.Open(index_pkl_file, 'rb'))['index'])
  assert FLAGS.num_nodes > 0

  dumps_dir = DatasetFileName('dumps')
  if not tf.gfile.Exists(dumps_dir):
    tf.gfile.MakeDirs(dumps_dir)
  ...
  if tf.gfile.Exists(directed_negs_file):
  ...
  def restore(suffix=''):
    if not tf.gfile.Exists(ModelFileName('embeddings.npy' + suffix)):
      print("Starting with fresh model...")
      return
    print("Restoring ...")

    EMBEDDINGS[:, :] = numpy.load(InFile('embeddings.npy' + suffix,'rb'))
