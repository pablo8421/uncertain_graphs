...
flags.DEFINE_integer('num_walks', 80, 'Number of walks per node.')
flags.DEFINE_integer('walk_length', 100,
...
def LargestUndirectedSubgraph(graph):
  ...
  cc = list(graph.subgraph(c).copy() for c in nx.connected_components(graph))
  sizes = map(len, cc)
...
def CreateDatasetFiles(graph, output_dir, partition=True,  num_walks=10, walk_length=80,
...
def SimulateWalks(train_graph, output_dir, num_walks=10, walk_length=80,
  ...
  for unused_j in range(FLAGS.num_walks):
